...
class bciApp(bciCore):
    def __init__(self):
        ...
        self.PHASES = [ {'name':'start','next':'test','duration':1},
                        {'name':'test','next':'relax','duration':4},
        ...
        if not DEBUG_WITHOUT_XIAOR:
            layout['camera'] = {'class':'MjpegStream','parm':{'size':(camera_w,camera_h),'position':(int(w/2),int(h/2)),
                            'anchor':'center','visible':True,'start':True,'url':'http://192.168.1.1:8080/?action=stream'}}
    ...
    def transition(self,phase):
        ...
        if phase == 'start':
            t = global_clock()
            self.marker.send_marker({'new-record':{'value':[1],'timestamp':[t]}})    #开始测试
    ...
    def stop_run(self):
        ...
        self.marker.send_marker({'endsigpro': {'value':[1],'timestamp':[global_clock()]},'end-record':{'value':[1],'timestamp':[global_clock()]}})  #测试完成
        self.gui.quit()
        self.ROBOT.close()
